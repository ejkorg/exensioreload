...
public class SenderService {
    ...
    public void runIfBelowThreshold() {
        ...
        // Read threshold and batch size from configuration (fallback to previous hard-coded values)
        long threshold = env != null ? env.getProperty("app.sender.threshold", Long.class, 600L) : 600L;
        if (pending > threshold) {
            log.info("Queue above threshold ({} > {}). Sender will not run.", pending, threshold);
            return;
        }
        int limit = env != null ? env.getProperty("app.sender.batch-size", Integer.class, 300) : 300;
        processBatch(limit);
    }
    ...
    public EnqueueResultHolder enqueuePayloadsWithResult(Integer senderId, java.util.List<String> payloadIds, String source) {
        ...
        // Optional enqueue throttling: if configured, refuse to enqueue when pending NEW items exceed configured max
        long prePending = repository.countByStatus("NEW");
        long maxPending = env != null ? env.getProperty("app.sender.max-pending-enqueue", Long.class, 0L) : 0L;
        if (maxPending > 0 && prePending >= maxPending) {
            log.info("Enqueue throttled: pending ({}) >= configured maxPending ({}). No items will be enqueued.", prePending, maxPending);
            return new EnqueueResultHolder(0, java.util.Collections.unmodifiableList(skipped), prePending, prePending);
        }
        ...
        long postPending = repository.countByStatus("NEW");
        log.info("Enqueued {} payloads (senderId={}, source={}), skipped {} already-present (pending before={}, after={})", inserted, senderId, source, skipped.size(), prePending, postPending);
        return new EnqueueResultHolder(inserted, skipped, prePending, postPending);
    }
    ...
    public int pushToExternalQueue(String site, Integer senderId, List<SenderQueueEntry> items) {
        // Guard: require explicit configuration to allow remote writes
            boolean allow = com.onsemi.cim.apps.exensio.resender.config.ConfigUtils.getBooleanFlag(env, "external-db.allow-writes", "EXTERNAL_DB_ALLOW_WRITES", false);
            if (!allow) {
    ...
    public long getSenderQueueCount(Integer senderId) {
    ...
    public static class EnqueueResultHolder {
        ...
        public final long pendingBefore;
        public final long pendingAfter;

        public EnqueueResultHolder(int enqueuedCount, java.util.List<String> skippedPayloads) {
            this(enqueuedCount, skippedPayloads, -1L, -1L);
        }

        public EnqueueResultHolder(int enqueuedCount, java.util.List<String> skippedPayloads, long pendingBefore, long pendingAfter) {
