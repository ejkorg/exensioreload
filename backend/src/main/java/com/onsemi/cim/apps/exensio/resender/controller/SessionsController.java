...
import com.onsemi.cim.apps.exensio.resender.repository.LoadSessionPayloadRepository;
import com.onsemi.cim.apps.exensio.resender.repository.LoadSessionRepository;
...
public class SessionsController {
    ...
    public String pushSessionToExternal(@PathVariable Long id) {
        ...
        java.util.List<com.onsemi.cim.apps.exensio.resender.entity.SenderQueueEntry> items = new java.util.ArrayList<>();
        for (LoadSessionPayload p : payloads) {
            com.onsemi.cim.apps.exensio.resender.entity.SenderQueueEntry e = new com.onsemi.cim.apps.exensio.resender.entity.SenderQueueEntry(s.getSenderId(), p.getPayloadId(), s.getSource());
            items.add(e);
        }
        int pushed = senderService.pushToExternalQueue(s.getSite(), s.getSenderId(), items);
        s.setPushedRemoteCount(pushed);
        s.setStatus("PUSHING_REMOTE"); // Set to PUSHING_REMOTE so completion checker will monitor it
        sessionRepo.save(s);
