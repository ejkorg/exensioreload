...
public class SessionPushService {
    ...
    public int pushSessionBatch(Long sessionId, int batchSize) {
        ...
        boolean useH2 = com.onsemi.cim.apps.exensio.resender.config.ConfigUtils.getBooleanFlag(env, "reloader.use-h2-external", "RELOADER_USE_H2_EXTERNAL", false);
            ...
            if (isOracle && !useH2) {
                ...
                for (LoadSessionPayload p : claimed) {
                    try {
                        ...
                        String meta = parts[0];
                        String data = parts[1];
                        if (meta == null || meta.isBlank() || data == null || data.isBlank()) {
                            p.markFailed("invalid payload: missing metadata_id or id_data");
                            p.setAttempts(p.getAttempts() + 1);
                            p.setNextAttemptAt(Instant.now().plusMillis(computeBackoffMillis(p.getAttempts())));
                            toSave.add(p);
                            continue;
