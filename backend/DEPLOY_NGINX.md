...
`/` and proxying `/api` to the backend. It includes a safe WebAuthn stub (if some
deployment layer injects `webauthnInterceptor.js`), a long-cache assets location, a
corrected `/api` proxy block (backend should own CORS), and the SPA route.
