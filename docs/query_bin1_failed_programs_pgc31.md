# Programs with Bin 1 Set as Failed — Program Class 31 Only (pgc_key = 31)

Query to retrieve all **program class 31** programs (`pgc_key = 31`) where bin 1 is designated as a fail bin, including the datetime the program was first loaded. Optional filters narrow results by fab and/or test facility.

**Note:** pgc_key = 31 is a custom/user-defined program class (reserved classes are 1–30). Verify in the `PROG_CLASS` table that this class exists and is configured for your use case.
...
WHERE 
    p.pgc_key = 31                  -- program class 31 only
    -- ========================================
...
| Filter | Purpose |
|---|---|
| `p.pgc_key = 31` | Restricts results to **program class 31** exclusively. Applied as a hard `WHERE` predicate — not optional. |
