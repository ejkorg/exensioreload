# Programs with Bin 1 Set as Failed — Final Summary Programs Only (pgc_key = 12)

Query to retrieve all **Final Summary programs** (`pgc_key = 12`) where bin 1 is designated as a fail bin, including the datetime the program was first loaded. Optional filters narrow results by fab and/or test facility.
...
WHERE 
    p.pgc_key = 12                  -- Final Summary programs only
    -- ========================================
...
| Filter | Purpose |
|---|---|
| `p.pgc_key = 12` | Restricts results to **Final Summary** program class exclusively. Applied as a hard `WHERE` predicate — not optional. |
...
- `pgc_key = 12` represents the **Final Summary** program class, typically used for high-level summary data across a fab or product.
