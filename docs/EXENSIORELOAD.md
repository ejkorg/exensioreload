...
| `cp.elasticsearch.url`                        | —                                       | Full `/_search` URL or base host; app auto-detects. Blank = ES disabled (no-op). |
| `cp.elasticsearch.cp-config-filter`           | `*sender*`                              | Wildcard applied to ES `cpConfig` field                                          |
| `cp.elasticsearch.enrichment-timeout-minutes` | 30                                      | Max wait in ENRICHMENT before FAILED                                             |
| `cp.elasticsearch.logRequestPayloads`         | `true` (dev) / `false` (oracle profile) | Log full ES query JSON — disable in production                                   |
...
16. **CP Elasticsearch polling** — `CpLogMonitor` is safe to deploy with ES unconfigured (`cp.elasticsearch.url` blank = no-op). Configure via `application.yml` under `cp.elasticsearch.*`. Uses JDK `java.net.http.HttpClient` — no extra Maven dependencies. `CP_ES_URL` accepts either a full search URL (e.g. `https://host/logs*dataport*/_search`) or a base host — `CpElasticsearchProperties.resolveSearchUrl()` auto-detects: if the URL already contains `/_search` it is used as-is; otherwise `/<index-pattern>/_search` is appended.
17. **No ES client library** — ES queries are raw HTTP POST to `/_search` using `java.net.http.HttpClient` + Jackson. Auth header (API key or Basic) is computed in `ElasticsearchLogService` constructor from `CpElasticsearchProperties`. The `cpConfig` wildcard filter value is driven by `cp.elasticsearch.cp-config-filter` (default: `*sender*`) — `ElasticsearchLogService` reads this from the properties bean; the value is no longer hardcoded.
