...
export class BackendService {
    ...
    listLocations(environment: string): Observable<ExternalLocationSummary[]> {
        return this.http.get<ExternalLocationSummary[]>(`${this.apiUrl}/environments/${encodeURIComponent(environment)}/locations`);
    }
    ...
    getHistoricalSenders(params: Record<string, any>): Observable<SenderOption[]> {
        return this.http.get<SenderOption[]>(`${this.apiUrl}/senders/historical/senders`, { params: this.toParams(params) });
    }
    ...
    listSenders(): Observable<SenderOption[]> {
        ...
        return new Observable<SenderOption[]>((observer: { next: (value: SenderOption[]) => void; complete: () => void }) => {
    ...
    getDiscoveryPreviewWithDuplicates(senderId: number, params: DiscoveryPreviewRequest): Observable<DiscoveryPreviewWithDuplicatesResponse> {
        return this.http.post<DiscoveryPreviewWithDuplicatesResponse>(`${this.apiUrl}/senders/${senderId}/discover/preview-with-duplicates`, params);
    }

    getHistoricalSummary(senderId: number, params: DiscoveryPreviewRequest): Observable<HistoricalPreviewSummary> {
        return this.http.post<HistoricalPreviewSummary>(`${this.apiUrl}/senders/${senderId}/discover/historical-summary`, params);
    }
    ...
    getStageRecordsList(site: string, senderId?: number, status?: string, page: number = 0, size: number = 50, requestId?: string): Observable<StageRecordPage> {
    ...
    getStagingSessions(page: number = 0,
                       size: number = 20,
                       filters?: {
                           q?: string;
                           senderId?: number | null;
    ...
    getStagingSessionFiles(sessionId: string,
                           page: number = 0,
                           size: number = 100,
                           status?: string,
                           q?: string): Observable<StageRecordPage> {
    ...
    getStagingSessionAnalytics(sessionId: string,
                               topPairs: number = 10,
                               startDate?: string,
                               endDate?: string): Observable<SessionAnalyticsResponse> {
        const params: any = { topPairs };
        if (startDate) params.startDate = startDate;
        if (endDate) params.endDate = endDate;
        return this.http.get<SessionAnalyticsResponse>(`${this.apiUrl}/stage/sessions/${encodeURIComponent(sessionId)}/analytics`, {
            params: this.toParams(params)
        });
    }

    refreshStagingSession(sessionId: string): Observable<StagingSessionDetail> {
        return this.http.post<StagingSessionDetail>(`${this.apiUrl}/stage/sessions/${encodeURIComponent(sessionId)}/refresh`, {});
    }
    ...
    getActiveStagingSessions(): Observable<ActiveStagingSession[]> {
        return this.getStagingSessions(0, 200).pipe(
            map((page: StagingSessionPage) => (page.items || []).map((item: StagingSessionSummary) => ({
                requestId: item.sessionId,
                site: item.site,
                senderId: item.senderId,
                senderName: item.senderName || '',
                status: (item.status === 'PARTIALLY_FAILED' ? 'ERROR' : item.status === 'COMPLETED' ? 'COMPLETED' : 'STAGING') as any,
        ...
        return this.getStagingSession(requestId).pipe(
            map((session: StagingSessionDetail) => ({
                session: {
                    ...
                    status: (session.status === 'PARTIALLY_FAILED' ? 'ERROR' : session.status === 'COMPLETED' ? 'COMPLETED' : 'STAGING') as any,
    ...
    bulkPauseMonitoring(senderIds: number[]): Observable<{ success: number; failed: number; message: string }> {
        return this.http.post<{ success: number; failed: number; message: string }>(
            `${this.apiUrl}/dashboard/bulk/pause`,
            { senderIds }
        );
    }
    ...
    bulkExportData(senderIds: number[], format: 'csv' | 'excel' = 'csv'): Observable<Blob> {
        return this.http.post(
            `${this.apiUrl}/dashboard/bulk/export`,
            { senderIds, format },
            { responseType: 'blob' }
        );
    }
    ...
    updateAlertThresholds(senderId: number, thresholds: AlertThreshold): Observable<AlertThreshold> {
        return this.http.put<AlertThreshold>(
            `${this.apiUrl}/alerts/sender/${senderId}/thresholds`,
            thresholds
        );
    }
    ...
    getSenderAlerts(senderId: number): Observable<SenderAlert[]> {
        return this.http.get<SenderAlert[]>(`${this.apiUrl}/alerts/sender/${senderId}`).pipe(
            catchError(() => of([]))
        );
    }
    ...
    getScheduledReports(): Observable<ScheduledReport[]> {
        return this.http.get<ScheduledReport[]>(`${this.apiUrl}/reports/scheduled`).pipe(
            catchError(() => of([]))
        );
    }
