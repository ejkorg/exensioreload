      ...
      <div class="lots-container" *ngIf="lots().length > 0; else noLots">
        <div class="lot-card glass-panel" *ngFor="let lot of lots()">
          ...
          <div class="lot-header" (click)="toggleLot(lot)">
            <div class="lot-info">
              <div class="lot-name">
                <app-glass-icon [name]="lot.expanded ? 'expand_more' : 'chevron_right'" [size]="20" color="muted">
                </app-glass-icon>
...
export class LotWaferProgressComponent {
  ...
  toggleLot(lot: LotProgress): void {
    const updated = this.lots().map((l: LotProgress) => (l.lot === lot.lot ? { ...l, expanded: !l.expanded } : l));
  ...
  getLotCountLabel(lot: LotProgress): string {
    ...
    const noun = hasRealWafer ? (count === 1 ? 'wafer' : 'wafers') : count === 1 ? 'file' : 'files';
