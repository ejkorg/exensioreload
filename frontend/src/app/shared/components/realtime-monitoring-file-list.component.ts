...
import { MonitoringPaginationService, MonitoringFileItem } from '../services/monitoring-pagination.service';
...
export class RealtimeMonitoringFileListComponent implements OnInit, OnDestroy {
  ...
  filteredFiles = computed(() => {
    ...
    if (search) {
      files = files.filter((f: MonitoringFileItem) =>
  ...
  private countByStatus(status: 'all' | 'READY' | 'ENQUEUED' | 'ENRICHMENT' | 'EXENSIO_LOADING' | 'COMPLETED' | 'ERROR') {
    return computed(() => {
  ...
  getStatusIcon(status: string): string {
    switch (status) {
      ...
      case 'ENQUEUED': return 'schedule_send';
      case 'ENRICHMENT': return 'hourglass_empty';
