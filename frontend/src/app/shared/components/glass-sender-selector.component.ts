...
import { GlassOption, GlassSelectComponent } from './glass-select.component';
    ...
    <div class="sender-selector" [class.sender-selector--auto-resolved]="autoResolved()">
      ...
      <div *ngIf="autoResolved() && selectedId() && !showDropdown()" class="sender-auto-resolved">
        ...
        <div class="sender-auto-resolved__value">
          ...
          <span
            *ngIf="selectedPort()"
            class="sender-port-badge"
            [title]="
              selectedPortSource() === 'dtp_sender' ? 'Port from DTP_SENDER table' : 'Fallback port from YAML config'
            "
          >
            Port: {{ selectedPort() }}
          </span>
        </div>
        <div class="sender-auto-resolved__note">Automatically matched based on your filter selection</div>
        <app-glass-button variant="secondary" size="small" (clicked)="showDropdown.set(true)">
      ...
      <div *ngIf="!autoResolved() && !loading() && !selectedId() && showFindButton()" class="sender-find">
        <app-glass-button variant="primary" size="medium" (clicked)="findSender.emit()" [disabled]="!canFind()">
          <app-glass-icon name="search" [size]="18"></app-glass-icon>
          Find Sender
        </app-glass-button>
        <p class="sender-find__hint">Click to search for matching senders based on your filters</p>
      </div>
    </div>
  `,
  styles: [
...
export class GlassSenderSelectorComponent {
  ...
  onSenderChange(senderId: number | null) {
    ...
    if (match) {
      ...
      // Use source from configuration service, fallback to port source for backward compatibility
      this.selectedPortSource.set((match as any).source ?? match.portSource ?? (match.port ? 'dtp_sender' : null));
    }
