...
import { BackendService, DashboardSnapshot, DashboardSiteSnapshot, DashboardSenderSnapshot, StagingSessionDetail, LimitsConfig } from '../api/backend.service';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatIconModule, MatButtonModule, MatTooltipModule, MatProgressBarModule, RouterModule, SparklineComponent, BulkActionsComponent, GlassCheckboxComponent],
...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    overallHealthScore = computed(() => {
        ...
        if (!s) return 0;
        const totalPayloads = s.global.ready + s.global.enqueued + s.global.completed;
        if (totalPayloads === 0) return 100;
        return Math.round((s.global.completed / totalPayloads) * 100);
    });
    ...
    resumeMonitoring() {
        this.router.navigate(['/resender/new'], { queryParams: { resume: '1' } });
    }
        ...
        this.backend.getDashboardSnapshot().subscribe({
            ...
            error: (err: unknown) => {
                ...
                const delayMs = Math.min(
                    this.retryBaseDelayMs * Math.pow(2, this.retryAttempts - 1),
                    30000
                );
    ...
    private detectChanges(previous: DashboardSnapshot | null, current: DashboardSnapshot | null): Set<string> {
        ...
        const trackedMetrics: Array<keyof DashboardSnapshot['global']> = ['backlog', 'ready', 'enqueued', 'completed', 'activeSenders'];
        ...
        current.sites.forEach((site) => {
            ...
            if (previousSite.metrics.backlog !== site.metrics.backlog || previousSite.metrics.completed !== site.metrics.completed || previousSite.metrics.ready !== site.metrics.ready || previousSite.metrics.enqueued !== site.metrics.enqueued) {
                changed.add(`site-${site.site}`);
            }
    ...
    getMetricHistory(metricKey: keyof MetricSnapshot): number[] {
        ...
        return this.metricHistory().map((snapshot: MetricSnapshot) => snapshot[metricKey]).slice(-60);
    }
