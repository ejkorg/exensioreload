...
import { BackendService, CoveragePoint, DashboardSiteSnapshot, DashboardSnapshot } from '../api/backend.service';
...
@Component({
    ...
    <div class="cov-shell">
      ...
      <section class="cov-filters glass-panel">
        ...
        <div class="filter-actions">
          ...
          <app-glass-button variant="secondary" [disabled]="points().length === 0" (clicked)="exportCsv()"
            >Export CSV</app-glass-button
          >
          <app-glass-button variant="primary" [disabled]="!selectedSite" [loading]="loading()" (clicked)="load()"
      ...
      <section class="kpi-strip" *ngIf="points().length > 0">
        ...
        <div class="kpi-card kpi-done">
          <span class="kpi-label">Done (PRD)</span>
          <span class="kpi-value">{{ kpis().done | number }}</span>
        </div>
        <div class="kpi-card kpi-done-sbx">
          <span class="kpi-label">Done (SBX)</span>
          <span class="kpi-value">{{ kpis().doneSbx | number }}</span>
        </div>
        <div class="kpi-card kpi-pending">
          <span class="kpi-label">Pending Verif.</span>
          <span class="kpi-value">{{ kpis().donePending | number }}</span>
        </div>
      ...
      <section class="chart-card glass-panel" *ngIf="points().length > 0">
        <div class="chart-header">
          ...
          <span
            >Stacked by status across {{ buckets().length }} {{ granularity }} bucket{{
              buckets().length !== 1 ? 's' : ''
            }}</span
          >
        </div>
      ...
      <section class="table-card glass-panel" *ngIf="points().length > 0">
        <div class="table-header-row">
          ...
          <label class="filter-label inline">
            <input
              class="cov-input cov-input--search"
              type="text"
              placeholder="Filter bucket or sender…"
              [(ngModel)]="tableSearch"
            />
          </label>
        </div>
        <div class="table-scroll">
          <table class="cov-table">
            <thead>
              <tr>
                <th (click)="sortBy('bucket')" class="sortable">
                  End-Time Bucket <span class="sort-icon">{{ sortIcon('bucket') }}</span>
                </th>
                <th (click)="sortBy('senderId')" class="sortable">
                  Sender <span class="sort-icon">{{ sortIcon('senderId') }}</span>
                </th>
                <th (click)="sortBy('site')" class="sortable">
                  Site <span class="sort-icon">{{ sortIcon('site') }}</span>
                </th>
                <th (click)="sortBy('total')" class="sortable num">
                  Total <span class="sort-icon">{{ sortIcon('total') }}</span>
                </th>
                <th (click)="sortBy('done')" class="sortable num">
                  Done PRD <span class="sort-icon">{{ sortIcon('done') }}</span>
                </th>
                <th (click)="sortBy('doneSbx')" class="sortable num">
                  Done SBX <span class="sort-icon">{{ sortIcon('doneSbx') }}</span>
                </th>
                <th (click)="sortBy('donePending')" class="sortable num">
                  Pending Verif. <span class="sort-icon">{{ sortIcon('donePending') }}</span>
                </th>
                <th (click)="sortBy('enqueued')" class="sortable num">
                  Enqueued <span class="sort-icon">{{ sortIcon('enqueued') }}</span>
                </th>
                <th (click)="sortBy('staged')" class="sortable num">
                  Staged <span class="sort-icon">{{ sortIcon('staged') }}</span>
                </th>
                <th (click)="sortBy('failed')" class="sortable num">
                  Failed <span class="sort-icon">{{ sortIcon('failed') }}</span>
                </th>
            ...
            <tbody>
              <tr *ngFor="let row of tableRows()">
                ...
                <td class="num done-sbx-cell">{{ row.doneSbx | number }}</td>
                <td
                  class="num pending-cell"
                  [title]="row.donePending > 0 ? 'Awaiting background verification or manual confirmation' : ''"
                >
                  {{ row.donePending | number }}
                </td>
                ...
                <td class="num">
                  <span
                    class="rate-pill"
                    [class.rate-full]="row.done + row.doneSbx + row.donePending === row.total"
                    [class.rate-partial]="
                      row.done + row.doneSbx + row.donePending > 0 &&
                      row.done + row.doneSbx + row.donePending < row.total
                    "
                    [class.rate-zero]="row.done + row.doneSbx + row.donePending === 0"
                  >
                    {{
                      row.total > 0
                        ? (((row.done + row.doneSbx + row.donePending) / row.total) * 100 | number: '1.0-0')
                        : 0
                    }}%
                  </span>
                </td>
              ...
              <tr *ngIf="tableRows().length === 0">
                <td colspan="11" class="empty-cell">No data matches your filter.</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  styles: [
    `
      ...
      .filter-grid app-glass-device-filter {
        width: 100%;
      }
      .filter-actions {
        display: flex;
      ...
      .cov-btn--primary:hover:not(:disabled) {
        background: linear-gradient(135deg, rgba(99, 102, 241, 1), rgba(139, 92, 246, 1));
      }
      .cov-btn--primary:disabled {
        opacity: 0.45;
        cursor: not-allowed;
      }
      .cov-btn--secondary {
      ...
      .cov-btn--secondary:hover:not(:disabled) {
        background: rgba(99, 102, 241, 0.3);
        border-color: rgba(167, 139, 250, 0.48);
      }
      .cov-btn--secondary:disabled {
        opacity: 0.35;
        cursor: not-allowed;
      }
      .mini-spinner {
      ...
      .kpi-enqueued .kpi-value {
        color: #fbbf24;
      }
      .kpi-staged {
        border-color: rgba(129, 140, 248, 0.28);
      }
      .kpi-staged .kpi-value {
        color: #a5b4fc;
      }
      .kpi-failed {
      ...
      .cov-table th {
        ...
        background: rgba(35, 25, 75, 0.9);
        border-bottom: 1px solid rgba(167, 139, 250, 0.18);
        white-space: nowrap;
      }
      .cov-table th.sortable {
        cursor: pointer;
        user-select: none;
      }
      .cov-table th.sortable:hover {
        color: #a5b4fc;
      }
      .sort-icon {
        font-size: 0.7rem;
...
export class CoverageComponent implements OnInit, OnDestroy {
  ...
  readonly kpis = computed(() => {
    ...
    const doneSbx = pts.reduce((s: number, p: CoveragePoint) => s + p.doneSbx, 0);
    const donePending = pts.reduce((s: number, p: CoveragePoint) => s + p.donePending, 0);
    ...
    const allDone = done + doneSbx + donePending;
    const rate = total > 0 ? Math.round((allDone / total) * 1000) / 10 : 0;
    return { total, done, doneSbx, donePending, enqueued, staged, failed, rate };
  });

  readonly tableRows = computed(() => {
    const q = this.tableSearch.toLowerCase();
    let rows = this.points().filter(
      (p: CoveragePoint) =>
  ...
  private loadSites(): void {
    this.backend.listAllSites().subscribe({
      next: (s: string[]) => {
        ...
        const allowed = this.authService.isAdminSignal() ? all : all.filter((site: string) => site.endsWith('-PROD'));
  ...
  onSiteChange(site: string): void {
    ...
    this.backend.getDashboardSnapshot().subscribe({
      next: (snap: DashboardSnapshot) => {
        const siteSnap: DashboardSiteSnapshot | undefined = snap?.sites?.find(
          (s: DashboardSiteSnapshot) => s.site === site,
        );
  ...
  exportCsv(): void {
    ...
    const header = 'bucket,senderId,site,total,donePrd,doneSbx,donePending,enqueued,staged,failed,rate\n';
    const body = rows
      .map((r: CoveragePoint) => {
        const allDone = r.done + r.doneSbx + r.donePending;
        const rate = r.total > 0 ? ((allDone / r.total) * 100).toFixed(1) : '0';
        return `${r.bucket},${r.senderId},${r.site},${r.total},${r.done},${r.doneSbx},${r.donePending},${r.enqueued},${r.staged},${r.failed},${rate}%`;
      })
      .join('\n');
  ...
  private renderChart(): void {
    ...
    const agg = new Map<
      string,
      { done: number; doneSbx: number; donePending: number; enqueued: number; staged: number; failed: number }
    >();
    for (const p of pts) {
      const cur = agg.get(p.bucket) ?? { done: 0, doneSbx: 0, donePending: 0, enqueued: 0, staged: 0, failed: 0 };
      agg.set(p.bucket, {
        done: cur.done + p.done,
        doneSbx: cur.doneSbx + p.doneSbx,
        donePending: cur.donePending + p.donePending,
    ...
    const doneSbx = buckets.map((b: string) => agg.get(b)?.doneSbx ?? 0);
    const donePending = buckets.map((b: string) => agg.get(b)?.donePending ?? 0);
    ...
    this.chart.setOption({
      ...
      legend: {
        data: ['Done (PRD)', 'Done (SBX)', 'Pending Verif.', 'Enqueued', 'Staged', 'Failed'],
        textStyle: { color: 'rgba(203,213,225,0.85)' },
        bottom: 0,
      },
      ...
      series: [
        { name: 'Done (PRD)', type: 'bar', stack: 'total', data: done, itemStyle: { color: '#10b981' } },
        { name: 'Done (SBX)', type: 'bar', stack: 'total', data: doneSbx, itemStyle: { color: '#34d399' } },
        { name: 'Pending Verif.', type: 'bar', stack: 'total', data: donePending, itemStyle: { color: '#a3e635' } },
