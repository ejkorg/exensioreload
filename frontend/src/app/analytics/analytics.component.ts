...
export class AnalyticsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private loadSnapshot(showLoading: boolean): void {
    ...
    this.backend.getDashboardSnapshot(selectedDevices.length > 0 ? selectedDevices : undefined).subscribe({
      ...
      series: [
        {
          ...
          type: 'line',
          smooth: true,
          data: history.map((p: AnalyticsSample) => p.backlog),
        ...
        },
        {
          ...
          type: 'line',
          smooth: true,
          data: history.map((p: AnalyticsSample) => p.ready),
        ...
        },
        {
          ...
          type: 'line',
          smooth: true,
          data: history.map((p: AnalyticsSample) => p.enqueued),
        ...
        },
        {
          ...
          type: 'line',
          smooth: true,
          data: history.map((p: AnalyticsSample) => p.completed),
  ...
  private renderSuccessRateChart(): void {
    ...
    const option: EChartsOption = {
      ...
      xAxis: {
        type: 'category',
        data: history.map((point: AnalyticsSample) => this.toShortTime(point.timestamp)),
        axisLabel: { color: '#9ca3af' },
      },
      yAxis: {
        type: 'value',
  ...
  private renderSiteVolumeChart(): void {
    ...
    const chartData = snap.sites.map((site: DashboardSiteSnapshot) => ({
      ...
      value:
        site.metrics.total > 0
          ? site.metrics.total
          : site.metrics.completed + site.metrics.ready + site.metrics.enqueued + site.metrics.backlog,
    }));
  ...
  private initChart(existing: ECharts | undefined, ref: ElementRef<HTMLDivElement>): ECharts {
    ...
    return (
      echarts.getInstanceByDom(ref.nativeElement) ?? echarts.init(ref.nativeElement, undefined, { renderer: 'canvas' })
    );
  }
  ...
  private collectTopSenders(snapshot: DashboardSnapshot, limit: number): DashboardSenderSnapshot[] {
    ...
    return allSenders.sort((a, b) => b.metrics.backlog - a.metrics.backlog).slice(0, limit);
  }
