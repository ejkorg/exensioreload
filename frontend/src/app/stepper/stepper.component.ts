...
export class StepperComponent implements OnInit, OnDestroy {
    ...
    private updateEnvOptions(roles: string[]): void {
        ...
        console.log('[StepperComponent.updateEnvOptions] roles:', JSON.stringify(roles), 'isAdmin:', isAdmin, 'hasUserOnly:', hasUserOnly);
    ...
    private getRowFamilyKeys(row: DiscoveryPreviewRow): Set<string> {
        ...
        const familyKeys = this.previewRows()
            .filter((r: DiscoveryPreviewRow) =>
                String(r.lot || '').trim() === lot &&
                String(this.getPreviewFilename(r) || '').trim() === filename
            )
    ...
    private normalizeTesterType(value: string | null | undefined): string | null {
        ...
        if (token === 'N/A' || token === 'NA' || token === 'ANY' || token === 'ALL' || token === 'NONE' || token === 'NULL') {
            return null;
        }
    ...
    fileTypeOptions = computed((): GlassOption[] => {
        ...
        return [
            ...
            ...sortedTypes.map((type: string) => ({ value: type, label: type }))
        ];
    });
    ...
    filteredPreviewRows = computed(() => {
        ...
        return source.map((row: DiscoveryPreviewRow & { originalFileName?: string | null }) => {
    ...
    someCurrentPageSelected = computed(() => {
        const currentPage = this.paginatedPreviewRows();
        return currentPage.some((r: DiscoveryPreviewRow) => this.selectedRows().has(this.getRowKey(r)))
    ...
    monitoringStats = computed(() => {
        ...
        const inQueueCount = hasFileBreakdown
            ...
            : (useWaferFallback ? waferRows.filter((w: WaferMonitoringRow) => w.status === 'ENQUEUED').length : (session?.filesEnqueued || 0));
        const enrichmentCount = hasFileBreakdown
            ? files.filter(f => f.status === 'ENRICHMENT').length
            : 0;
        const exensioLoadingCount = hasFileBreakdown
            ? files.filter(f => f.status === 'EXENSIO_LOADING').length
            : 0;
    ...
    monitoringFiles = computed(() => {
        return this.monitoringWaferRows().map((row: WaferMonitoringRow, index: number) => ({
    ...
    constructor(
        ...
        this.userRolesSubscription = this.authService.user$.subscribe((user: { username: string; roles: string[] } | null) => {
        ...
        effect(() => {
            ...
            if ((status === 'COMPLETED' || status === 'PARTIALLY_FAILED' || status === 'CANCELLED') && !this.sessionCompletedNotified()) {
                const total = session.totalFiles || 0;
                const completed = session.filesDone || 0;
                const failed = session.filesFailed || 0;
                this.toast.success(`Session ${session.sessionId} completed: ${completed}/${total} succeeded, ${failed} failed`);
    ...
    private setupReactiveSenderLookup() {
        effect(() => {
            ...
            const shouldTrigger = this.shouldTriggerSenderLookup(
                site, location, dataType, testerType, testPhase, historical, testerTypes, testerTypesLoading
            );
    ...
    private setupSenderAutoLookup() {
        this.senderLookupSubscription = this.senderLookupSubject.pipe(
    ...
    private loadLocationsOnly(site: string) {
        ...
        this.backend.getDistinctLocations({ connectionKey: site, environment: (this.selectedEnv() || 'QA').toLowerCase() }).subscribe({
            next: (locs: string[]) => {
    ...
    private loadTesterTypesForDataType() {
        ...
        this.backend.getDistinctTesterTypes(params).subscribe({
            next: (tts: string[]) => {
                ...
                const filtered = this.isAdminUser()
                    ? unique
                    : unique.filter(t => t.toUpperCase() !== 'QA');
    ...
    private loadTestPhasesForFilters() {
        ...
        this.backend.getDistinctTestPhases(params).subscribe({
            next: (phases: string[]) => {
                const filtered = this.normalizeTestPhases(phases || [], this.selectedDataType(), this.normalizeTesterType(this.selectedTesterType()));
    ...
    removePair(index: number) {
        this.lotWaferPairs.update((pairs: Array<{ lot: string; wafer: string }>) => pairs.filter((_: { lot: string; wafer: string }, i: number) => i !== index));
    }
    ...
    loadPreview() {
        ...
        if (!this.canProceedToPreview()) {
            this.toast.warning('Please select all required filters (Site, Location, Data Type) and at least one additional filter');
            return;
        }
        ...
        if (useBypassCap) {
            this.toast.info(
                'Running high-volume discovery query (no preview cap). This may take longer to complete.',
                5000
            );
        }
    ...
    totalPages = computed(() => {
        ...
        const total = this.allPreviewRows().length > 0
            ? this.filteredPreviewRows().length
            : this.previewTotal();
    ...
    stageSelected(forceDuplicates: boolean = false) {
        ...
        console.log('[STAGING] Creating session...', { site: selectedSite, senderId, senderName: this.getSelectedSenderName(), environment: resolvedEnvironment });

        this.stagingSession.createSession(selectedSite, senderId, this.getSelectedSenderName(), resolvedEnvironment).subscribe({
                ...
                this.backend.stagePayloads(senderId, body).subscribe({
                    next: (response: any) => {
                        ...
                        if (response?.requiresConfirmation && !forceDuplicates) {
                            ...
                            this.showDuplicateConfirmation(response, payloads.length, {
    ...
    stageAllMatching(forceDuplicates: boolean = false) {
        ...
        dialogRef.afterClosed().then((result: any) => {
            ...
            console.log('[STAGING] Creating session for stage-all...', { site: selectedSite, senderId, senderName: this.getSelectedSenderName(), environment: resolvedEnvironment });
                ...
                next: (session: CreateSessionResponse) => {
                    ...
                    if (localRows.length > 0 && localRows.length >= this.previewTotal()) {
                        ...
                        this.backend.stagePayloads(senderId, body).subscribe({
                            ...
                            error: (err: any) => {
                                ...
                                this.toast.error(`Background staging error: ${err?.error?.message || err?.statusText || 'Unknown error'}`, 7000);
                            }
                        });
                        return;
                    }
                    ...
                    this.backend.stageAll(senderId, body).subscribe({
                        ...
                        error: (err: any) => {
                            ...
                            this.toast.error(`Background staging error: ${err?.error?.message || err?.statusText || 'Failed to stage all payloads'}`, 7000);
                        }
                    });
                },
                error: (err: any) => {
    ...
    private showDuplicateConfirmation(
        ...
        selectedPayloads: Array<{ metadataId: string; dataId: string; lot?: string | null; wafer?: string | null; filename?: string | null }>
    ) {
        const rawDuplicates = response?.duplicatePayloads || [];
        const duplicateSourceByKey = new Map<string, { lot?: string | null; wafer?: string | null; filename?: string | null }>();
        ...
        dialogRef.afterClosed().then((result: any) => {
            ...
            if (confirmed === true) {
                ...
                const targetDuplicates = selectedKeys
                    ? duplicates.filter((dup: DuplicatePayloadInfo) =>
                        !!dup.metadataId && selectedKeys.has(`${dup.metadataId}::${dup.dataId}`)
                      )
            ...
            this.backend.dispatch({ site: context.site, senderId: context.senderId, limit: null }).subscribe({
                next: () => {
                    this.completeStep(1);
                    this.currentStep.set(2);
                    this.toast.info(`Skipped ${duplicateCount} duplicate payload${duplicateCount === 1 ? '' : 's'}. Continuing with ${stagedCount} staged payload${stagedCount === 1 ? '' : 's'}.`, 6000);
                    this.startMonitoring();
                },
                error: (err: any) => {
                    ...
                    this.completeStep(1);
                    this.currentStep.set(2);
                    this.toast.warning(`Duplicates were skipped. ${stagedCount} payload${stagedCount === 1 ? '' : 's'} staged, but dispatch failed. Please retry dispatch.`, 7000);
                    this.startMonitoring();
                }
            });
        });
    ...
    private handleStageAllResponse(
        ...
        if (duplicateCount > 0) {
            this.toast.success(`Staged ${stagedCount} payload${stagedCount === 1 ? '' : 's'} (${duplicateCount} duplicate${duplicateCount === 1 ? '' : 's'} skipped)`, 6000);
        } else {
            this.toast.success(`Successfully staged ${stagedCount} payload${stagedCount === 1 ? '' : 's'}`, 5000);
        }
        ...
        if (stagedCount <= 0) {
            ...
            if (dupCount > 0) {
                ...
                this.toast.info(`These ${total} file${total === 1 ? ' is' : 's are'} already staged and queued. Opening monitoring to track progress.`, 6000);
    ...
    private handleStageAllBackgroundResponse(response: any) {
        ...
        console.log('[STAGING] Background stage-all completed:', { stagedCount, requeuedCount, duplicateCount, requiresConfirmation });
            ...
            if (senderId && site) {
                this.backend.dispatch({ site, senderId }).subscribe({
                    next: () => {
                        ...
                        if (sessionId) { this.stagingSession.refreshSessionFiles(sessionId); }
                    },
                    error: () => {}
                });
            }
    ...
    onExportCsvClick() {
        const files = this.stagingSession.sessionFiles().map((item: StageRecordView) => ({
            id: item.id || 0,
    ...
    private expandMonitoringRows(selectedRows: DiscoveryPreviewRow[], allRows: DiscoveryPreviewRow[]): DiscoveryPreviewRow[] {
        ...
        const selectedFamilies = new Set(
            selectedRows.map((row) => `${String(row.lot || '').trim()}::${String(this.getPreviewFilename(row) || '').trim()}`)
        );
    ...
    private performHistoricalSenderLookup() {
        ...
        return this.backend.getHistoricalSenders(params).pipe(
            map((candidates: SenderOption[]) => {
                ...
                const filtered = (candidates || []).filter((s: SenderOption) =>
                    !(s.name || '').toUpperCase().endsWith(oppositeSuffix)
                );
                ...
                if (filtered.length === 1) {
                    ...
                    this.senderAutoResolved.set(false);
                    this.senderFallback.set(true);
                    this.toast.warning('No historical sender matched. Verify a HIST sender exists in Dataport for this data type/env.');
                }
